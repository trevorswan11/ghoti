...
//
// Owns one persistent `analysis_session` spanning every file ever opened
class document_store {
  ...
  public:
    explicit document_store(std::ostream& error_stream) noexcept
    ...
    // The persistent module graph, spanning every known root
    [[nodiscard]] auto manager() const -> const mod::module_manager& {
