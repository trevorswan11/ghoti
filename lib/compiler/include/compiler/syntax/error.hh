#pragma once

#include <utility>

#include <stdx/result.hh>
#include <stdx/types.hh>

#include "support/diagnostic.hh"

namespace ghoti::syntax {

enum class error : u8 {
    UNEXPECTED_TOKEN,
    MISSING_PREFIX_PARSER,
    INFIX_MISSING_RHS,
    ILLEGAL_IDENTIFIER,
    EMPTY_RAW_IDENTIFIER,
    UNTERMINATED_RAW_IDENTIFIER,
    END_OF_TOKEN_STREAM,
    DECL_MISSING_VALUE,
    EMPTY_FILE_IMPORT,
    ILLEGAL_IMPORT_TYPE,
    FILE_IMPORT_MISSING_ALIAS,
    DUPLICATE_DECL_MODIFIER,
    ILLEGAL_DECL_MODIFIERS,
    INTEGER_OVERFLOW,
    FLOAT_OVERFLOW,
    DOUBLE_OVERFLOW,
    UNKNOWN_CHARACTER_ESCAPE,
    PREFIX_MISSING_OPERAND,
    INDEX_MISSING_EXPRESSION,
    DISCARD_MISSING_DISCARDEE,
    EMPTY_LOOP,
    WHILE_MISSING_CONDITION,
    INVALID_MEMBER,
    EXTERN_VALUE_INITIALIZED,
    ILLEGAL_LOOP_NON_BREAK,
    FOR_MISSING_ITERABLES,
    FOR_ITERABLE_CAPTURE_MISMATCH,
    EMPTY_WHILE_CONTINUATION,
    IF_MISSING_CONDITION,
    ILLEGAL_IF_BRANCH,
    MISSING_ARRAY_SIZE_TOKEN,
    MATCH_EXPR_MISSING_CONDITION,
    ARMLESS_MATCH_EXPR,
    ILLEGAL_MATCH_PATTERN,
    ILLEGAL_MATCH_ARM,
    ILLEGAL_MATCH_CATCH_ALL,
    FN_PARAMETER_HAS_DEFAULT_VALUE,
    FN_PARAMETER_IS_NORETURN,
    ILLEGAL_EXPLICIT_TYPE,
    MISSING_EXPLICIT_TYPE,
    EXPLICIT_FN_TYPE_HAS_BODY,
    FN_TYPE_PARAMETER_UNNAMED,
    FN_TYPE_PARAMETER_DISCARDED,
    FN_DECLARATION_WITHOUT_BODY,
    ILLEGAL_FUNCTION_TYPE_MODIFIER,
    ILLEGAL_SELF_PARAMETER_MODIFIER,
    ILLEGAL_NORETURN_TYPE_MODIFIER,
    ILLEGAL_VOID_TYPE_MODIFIER,
    ILLEGAL_TYPE_TYPE_MODIFIER,
    COMMA_WITH_MISSING_CALL_ARGUMENT,
    EMPTY_UNION,
    EMPTY_ENUM,
    ILLEGAL_NON_EXHAUSTIVE_ENUM,
    ILLEGAL_DEFERRED_STATEMENT,
    DEFER_MISSING_DEFERREE,
    EMPTY_TEST_DESCRIPTION,
    VALUED_BREAK_MISSING_LABEL,
    VALUED_CONTINUE,
    ILLEGAL_LABEL,
    ILLEGAL_LABEL_EXPRESSION,
    ILLEGAL_LABEL_STATEMENT,
    ILLEGAL_MODIFIED_TYPE_LITERAL,
    ILLEGAL_MOVE_USAGE,
    EXPRESSION_NESTED_TOO_DEEPLY,
    UNTERMINATED_STRING,
    INVALID_NUMBER_LITERAL,
    INVALID_CHARACTER_LITERAL,
    EMPTY_EXTERN_TARGET,
    ASM_MISSING_TEMPLATE,
    ASM_DUPLICATE_CLAUSE,
    ASM_UNKNOWN_CLAUSE,
    ASM_UNKNOWN_OPTION,
    ASM_MALFORMED_OPERAND,
    CFG_MISSING_PREDICATE,
    CFG_MISSING_BODY,
    CFG_ELSE_WITHOUT_CFG,
    CFG_VALUE_MISSING_ARGUMENT,
    CFG_VALUE_DUPLICATE_FALLBACK,
    CFG_VALUE_EMPTY_GUARD,
    ILLEGAL_INTERFACE_MEMBER,
    IMPL_MISSING_TARGET,
    PACK_PARAM_NOT_LAST,
    PACK_EXPANSION_MISPLACED,
    CONSTEXPR_LOOP_HAS_ELSE,
    CONSTEXPR_LOOP_LABELED,
};

using diagnostic  = diagnostic<error>;
using diagnostics = diagnostic_list<diagnostic>;

template <typename... Args>
[[nodiscard]] constexpr auto make_syntax_err(Args&&... args) -> stdx::err<diagnostic> {
    return stdx::make_err<diagnostic>(std::forward<Args>(args)...);
}

} // namespace ghoti::syntax
