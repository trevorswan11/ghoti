#pragma once

#include <utility>

#include <stdx/result.hh>
#include <stdx/types.hh>

#include "support/diagnostic.hh"

namespace ghoti::sema {

enum class error : u8 {
    IDENTIFIER_REDECLARATION,
    ILLEGAL_TOP_LEVEL_STATEMENT,
    ILLEGAL_CONTROL_FLOW,
    ILLEGAL_IMPORT_LOCATION,
    ILLEGAL_NON_CONST_STATEMENT,
    FUNCTION_DECLARATION_MISSING_BODY,
    REDUNDANT_CONSTEXPR,
    MUTABLE_TYPE_BINDING,
    INVALID_TABLE_IDX,
    SHADOWING_DECLARATION,
    ILLEGAL_TEST_LOCATION,
    MODULE_LOAD_ERROR,
    NON_CALLABLE_EXPRESSION,
    ARITY_MISMATCH,
    UNDECLARED_IDENTIFIER,
    TYPE_MISMATCH,
    OUTER_SCOPE_NOT_FOUND,
    CYCLIC_DEPENDENCY,
    ILLEGAL_SELF_PARAMETER,
    ILLEGAL_LABEL_USAGE,
    DUPLICATE_TEST_NAME,
    DUPLICATE_FIELD,
    MISSING_FIELD,
    UNKNOWN_FIELD,
    DUPLICATE_ENUMERATION,
    UNKNOWN_ENUMERATION,
    ILLEGAL_MATCH_PATTERN,
    AUTO_WITHOUT_INITIALIZER,
    ILLEGAL_AUTO_USAGE,
    CONSTEXPR_EVALUATION_FAILED,
    CONSTEXPR_RECURSION_LIMIT_EXCEEDED,
    GIR_FORBIDDEN_TYPE,
    ILLEGAL_PRIVATE_ACCESS,
    ASSIGNMENT_TO_CONST,
    ILLEGAL_CONST_CAST,
    OPERATOR_TYPE_MISMATCH,
    RETURN_TYPE_MISMATCH,
    AUTO_RETURN_TYPE_CONFLICT,
    ILLEGAL_OPAQUE_TYPE,
    ILLEGAL_REFERENCE_TO_REFERENCE,
    ILLEGAL_REFERENCE_FIELD,
    CLOSURE_SIGNATURE_MISMATCH,
    ILLEGAL_CLOSURE_ESCAPE,
    ILLEGAL_RECURSIVE_CLOSURE,
    IMPORTED_MODULE_CONTAINS_ERRORS,
    ILLEGAL_RVALUE_CAPTURE,
    ILLEGAL_FIELD_ORDER_DEPENDENCY,
    ILLEGAL_INLINE_ASM,
    CFG_UNKNOWN_ATOM,
    CFG_UNKNOWN_ENUM_MEMBER,
    CFG_COMPARISON_TYPE_MISMATCH,
    CFG_ILLEGAL_PREDICATE,
    CFG_ILLEGAL_CFG_VALUE_REFERENCE,
    CFG_VALUE_CYCLE,
    CFG_VALUE_GUARD_TYPE_MISMATCH,
    CFG_VALUE_MISSING_FALLBACK,
    CFG_NO_MATCHING_ARM,
    COMPILE_ERROR_REACHED,
    COMPILE_ERROR_NON_CONSTANT_MESSAGE,
    STATIC_ASSERTION_FAILED,
    UNWRAP_ON_NON_RESULT,
    UNWRAP_RETURN_TYPE_MISMATCH,
    UNWRAP_OUTSIDE_FUNCTION,
    DEFER_BODY_JUMP,
    ERRDEFER_IN_INFALLIBLE_FN,
    ERRDEFER_MUTABLE_CAPTURE,
    TEST_BUILTIN_OUTSIDE_TEST,
    ILLEGAL_LOCAL_DECL_MODIFIER,
    UNUSED_RESULT,
    ILLEGAL_DISCARDABLE,
    ILLEGAL_OPEN_RANGE,
    ORPHAN_IMPL,
    DUPLICATE_IMPL,
    DUPLICATE_MEMBER,
    UNKNOWN_IMPL_MEMBER,
    MISSING_IMPL_METHOD,
    IMPL_SIGNATURE_MISMATCH,
    IMPL_SELF_MISMATCH,
    IMPL_FIELD_INJECTION,
    INTERFACE_NOT_A_VALUE,
    UNSATISFIED_BOUND,
    AMBIGUOUS_METHOD,
    CONFLICTING_ASSOC,
    SEALED_METHOD,
    ASSOC_CONST_SELF_DEP,
    ILLEGAL_UNSIZED_TYPE,
    DYN_UNBOUND_ASSOC,
    DYN_BY_VALUE_SELF,
    STATIC_REQUIREMENT_UNSUPPORTED,
    UNSUPPORTED_TARGET,
    ILLEGAL_PACKED_FIELD,
    ILLEGAL_PACKED_FIELD_ADDRESS,
    LITERAL_OUT_OF_RANGE,
    CONCAT_REQUIRES_ARRAY_OR_SLICE,
    CONCAT_ELEM_TYPE_MISMATCH,
    CONCAT_NOT_FOLDABLE,
    CONSTEXPR_VAR_NOT_FOLDABLE,
    CONSTEXPR_VAR_ASSIGN_NOT_FOLDABLE,
    CONSTEXPR_VAR_ADDRESS_OF,
    // Parses; semantics land in a later phase. Gates resolution to fail clean, not crash.
    PACK_PARAM_NOT_YET_SUPPORTED,
    PACK_USE_OUT_OF_POSITION,
    PACK_INDEX_OUT_OF_RANGE,
    PACK_INDEX_NOT_CONST,
    CONSTEXPR_LOOP_COUNT_NOT_STATIC,
    CONSTEXPR_LOOP_BREAK,
    CONSTEXPR_LOOP_CONTINUE,
    CONSTEXPR_WHILE_NONFOLDABLE_COND,
    CONSTEXPR_LOOP_LIMIT,
    FIELD_NOT_FOUND,
    PACK_EXPANSION_MISPLACED,
    FIELD_HAS_NO_DEFAULT,
};

using diagnostic  = diagnostic<error>;
using diagnostics = diagnostic_list<diagnostic>;

template <typename... Args>
[[nodiscard]] constexpr auto make_sema_err(Args&&... args) -> stdx::err<diagnostic> {
    return stdx::make_err<diagnostic>(std::forward<Args>(args)...);
}

} // namespace ghoti::sema
